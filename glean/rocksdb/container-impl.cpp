...
const Family Family::keys("keys", 0);
...
ContainerImpl::ContainerImpl(
    ...
    folly::Optional<std::shared_ptr<Cache>> cache) : db(nullptr, &mdb_env_close) {
  mode = m;

  unsigned int flags =
      (m == Mode::ReadOnly ? MDB_RDONLY : 0) |
      MDB_NOTLS;
  ...
  LOG(INFO) << folly::sformat(
      "max key size: {}", mdb_env_get_maxkeysize(db_ptr));
...
void ContainerImpl::writeData(folly::ByteRange key, folly::ByteRange value) {
  requireOpen();
  Txn txn = txn_write();
  txn.put(family(Family::meta), slice(key), slice(value));
  txn.commit();
}
...
bool ContainerImpl::readData(
  ...
  MDB_val val;
  Txn txn = txn_read();
  if (txn.get(family(Family::meta), slice(key), val)) {
    f(byteRange(val));
    return true;
  } else {
    return false;
